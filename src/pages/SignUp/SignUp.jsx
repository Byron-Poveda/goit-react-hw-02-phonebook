...
function SignUp() {
  ...
  const submitForm = (e) => {
    e.preventDefault()
    if (!firstName || !email || !password) return Notify.warning("Please fill all fields")
    if(password.length < 6) {
      setPassword('')
      setErrors('The password must be greater than ( 7 ) digits')
      return
    }
    dispatch(signUpUser({ name: `${firstName + lastName}`,email, password }))
    setFirstName('')
    setLastName('')
    setEmail('')
    setPassword('')
          ...
          <div className="md:flex w-full">
              ...
              <div className="w-full md:w-1/2 py-10 px-5 md:px-10 flex items-center flex-col justify-center">
                  ...
                  <form onSubmit={submitForm} className='flex flex-col gap-[30px]'>
                      ...
                      <div className="flex mb-[10px] flex-col gap-[20px]">
                          <div className="w-full">
                            <Input 
                                modelValue={password}
                                required
